...
//  Kdelem_ptr temp_elem;
  ...
  for(i = 2; i<(numofblocks*2); i++)
  {
	  ...
	  if(permindex >= 0){
	      ...
	      if(elem->region[dimensions].end==21816721+shiftvalue){
		  // printf("HALLO %d %u\n",permindex,rootarray[permindex]);
		  //printf("HALLO %d %u\n",perm[index_of_current_chain_root]->region[0].start,elem->region[dimensions].end);
	  ...
	  if((permindex < 0)||((flag==1)&&(perm[permindex]->num==0))||(perm[permindex]->num==0)||(Rewindflag==1)) //no such element
	  {
		  ...
		  if(elem->region[dimensions].end==21816721+shiftvalue){
		      //  printf("HALLO %d %u\n",rootarray[sorted_list[i]],index_of_current_chain_root);
		      // printf("HALLO %d %u\n",perm[index_of_current_chain_root]->region[0].start,elem->region[dimensions].end);
		      //printf("%u %u\n",sorted_list[i],rootarray[sorted_list[i]]);    
		  }
...
// Append information to the statistics file .stc
      ...
      printf("# no. of chains: %u \n",chain_counter);
      fprintf(stcptr,"# no. of chains: %u \n",chain_counter);
      fclose(stcptr);
//
    free(regionend);
}
...
//  char tempstr[MaxFileNameSize];
  ...
  long* positiondigitarray=(long*)malloc(sizeof(long)*(dimensions+1));
  for(i=0;i<=dimensions;i++){
      positiondigitarray[i]=2+(long)log10((double)genome_sizes_array[i]);
      leastscore=leastscore+((double)genome_sizes_array[i]);
  }
  long fragmentnumdigits=2+(long)log10((double)numofblocks);
...
void kdChainer::storeMultiChainDB(ProcessFragmentFile* InfoObj,int IncludeDirection){
    ...
    long fragmentnumdigits=2+(long)log10((double)numofblocks);
    long scorenumdigit=7+1+(long)log10(max(maxscore_gapti,max_fragment_weight));
...
void kdChainer::storeMultiContigDB(ProcessFragmentFile* InfoObj,int IncludeDirection){
   ...
   //Kdelem_ptr local_tail=NULL;
    ...
    long fragmentnumdigits=1+(long)log10((double)numofblocks);
    long contignumdigits=1+(long)log10(contigobj->number_of_contigs);
    long scorenumdigit=7+1+(long)log10(max(maxscore_gapti,max_fragment_weight));
...
void kdChainer::storeMultiEstDB(ProcessFragmentFile* InfoObj,int IncludeDirection){
    ...
    long fragmentnumdigits=1+(long)log10((double)numofblocks);
    long contignumdigits=1+(long)log10(contigobj->number_of_contigs);
    long scorenumdigit=7+1+(long)log10(global_maxscore);
