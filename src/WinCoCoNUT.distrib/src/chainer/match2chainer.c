...
//  long kk=0;
  ...
  if(argc!=3)
    ...
    printf("Usage: match2chainer -{filetype} filename \n");
    ...
    printf("Output:\n ");
    printf("  -One file: 'filename.mat' in case of multimat input file \n");
    printf("   -Two files: (1) 'filename.mat'  containing direct matches.\n");  
    printf("               (2)'filename.rev.mat' containing  palindromic matches. \n");
    return(FAIL);
  } 
  ...
  if(filetype==1){
	  ...
	  if (strcmp(token,"#")==0) {
		  ...
		  if(strcmp(token,"databaselength")==0){
		      ...
		      if(strcmp(token,"(including")==0){
			  token = strtok ( NULL, " ");
			  //query_length=query_length-atol(token);
			  //  printf("Sep %s\n",token); 
		      }
		      break; 
		  }
